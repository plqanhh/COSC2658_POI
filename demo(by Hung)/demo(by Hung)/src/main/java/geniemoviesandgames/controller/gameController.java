...
public class gameController extends Switchingscene implements Initializable {
    ...
    @FXML
    private ObservableList<game> searchedGameItem;
...
protected void showTextOnTextField(MouseEvent e){
        item selectedItem = gameTableView.getSelectionModel().getSelectedItem();
...
//        listOfItems.remove();
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        ObservableList<game> gameItems = gameTableView.getItems();
        FilteredList<game> filteredGame = new FilteredList<>(gameItems, b->true);
        searchTextField.textProperty().addListener((observable,oldValue,newValue) ->{
            filteredGame.setPredicate(game -> {
                ...
                if (game.getItemTitle().toLowerCase().contains(lowerCaseFilter)) {
                    gameTableView.setItems(filteredGame);
                    return true;
