...
public class oldMovieRecordController extends Switchingscene implements Initializable {
    ...
    @FXML
    private TextField searchMovieTextField;
    @FXML
...
//movies ds = new movies("f", "f", true, 2, 2.5, "s");
    ...
    protected void showTextOnTextField(MouseEvent e){
        movies selectedItem = movieTableView.getSelectionModel().getSelectedItem();
...
//        FXMLLoader edit = new FXMLLoader(getClass().getResource("/geniemoviesandgames/screen_design/editBarForMovieAndDVD.fxml"));
        ...
        ObservableList<movies> movieItems = movieTableView.getItems();
        FilteredList<movies> filteredGame = new FilteredList<>(movieItems, b->true);
        searchMovieTextField.textProperty().addListener((observable,oldValue,newValue) ->{
            filteredGame.setPredicate(game -> {
                ...
                if (game.getItemTitle().toLowerCase().contains(lowerCaseFilter)) {
                    movieTableView.setItems(filteredGame);
                    return true;
